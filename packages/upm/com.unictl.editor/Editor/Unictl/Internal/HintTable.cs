...
namespace Unictl.Internal
{
    public static class HintTable
    {
        private static readonly Dictionary<string, string> _hints = new()
        {
            ...
            { "timeout",                "client wait timeout; build still running; poll: unictl build status --job-id <id>" },
            ...
            { "custom_build_failed",    "Inspect the custom build method; it reported failure through UnictlBuildContext." },
            { "custom_build_exception", "Inspect the custom build method; it threw before reporting terminal status." },
            ...
            { "editor_log_unavailable", "Project-scoped editor log is unavailable. Restart the editor with: unictl editor restart" },
            { "editor_log_project_log_missing", "Project-scoped editor log is missing. Restart the editor with: unictl editor restart" },
            { "editor_log_stale_session", "Project-scoped editor log belongs to an older session. Restart the editor with: unictl editor restart" },
