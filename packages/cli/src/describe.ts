...
export const v07Describes: Record<string, DescribeMetadata> = {
  ...
  "deploy.android.keystore.set": {
    ...
    summary: "Configure Android keystore credentials. Secrets redacted from --describe output.",
    when: [
    ...
    args: [
      ...COMMON_ARGS,
      { name: "path", type: "path", required: true, description: "Keystore file path" },
      { name: "alias", type: "string", required: true, description: "Key alias" },
      { name: "keystore-pass", type: "string", required: false, description: "Keystore password (stdin if omitted)" },
      { name: "key-pass", type: "string", required: false, description: "Key password (stdin if omitted)" },
    ],
    examples: [
      { cmd: "unictl deploy android keystore set --path Build/release.keystore --alias release", intent: "configure release signing" },
    ],
    exit_codes: [0, 2, 78],
  ...
  "settings.raw-set": {
    ...
    examples: [
      { cmd: "unictl settings raw-set m_CompanyName Tinycell --no-warranty", intent: "edit a non-lifecycle-sensitive setting" },
    ],
    exit_codes: [0, 2, 78],
