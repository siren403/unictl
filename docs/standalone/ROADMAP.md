...
`v0.4.0` 시점 기준 후속 로드맵. 확정된 방향과 아이디어 수준을 분리한다.
...
## v0.5.0 — Minor (Discoverability 완성)
...
- **Release.ts `[Unreleased]` 자동 승격**: 릴리즈 시 `[Unreleased]` → `[<version>] - <date>`로 자동 이동 + 새 `[Unreleased]` 생성. 현재는 수동.
